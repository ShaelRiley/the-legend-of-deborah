-- B9 production gate exercises progression, rewards, placement and canonical spawning.
...
s.CampaignEpoch=1;s.RunId='b9';s.CampaignSeed=77;s.LevelSeed=123
...
local ids={'towline','screenwright'}
local expected={towline={die=8,xp=50},screenwright={die=10,xp=60}}
...
pair('towline');D.Entities={};D.activeCount=0
local creates=0
...
local encounter={id=903,cell=cell,cellKey=key(3,3,0),role='arena',sector=2,composition={towline=1,screenwright=1},entities={}}
...
for _,name in ipairs({'towline_detail','screenwright_detail'}) do
    ...
    assert(not table.HasValue(D:_EligibleTemplates(3,'reward'),name),'tether/cover specialist does not enter reward-only path')
    for seed=1,16 do
        local composition=D:_TemplateComposition(name,LOD.RNG.New(seed),4)
        for _,id in ipairs(ids) do
            if composition[id] then assert(composition[id]==1,'party/depth enrichment caps tether/cover source') end
        end
    end
end
for _,id in ipairs(ids) do
    assert(E:Placement(s.Graph,cell,id,'arena'),'ordinary legal room admits tether/cover source')
    s.Graph.CellTags[key(3,3,0)]={safe=true}
    assert(not E:Placement(s.Graph,cell,id,'arena'),'safe room rejects tether/cover source')
    s.Graph.CellTags[key(3,3,0)]={objective=true}
    assert(not E:Placement(s.Graph,cell,id,'arena'),'objective room rejects tether/cover source')
    s.Graph.CellTags[key(3,3,0)]={}
    s.Graph.VerticalEdges={{a=cell,b=s.Graph.Cells[key(4,3,0)]}}
    assert(not E:Placement(s.Graph,cell,id,'arena'),'vertical transition rejects tether/cover source')
    s.Graph.VerticalEdges={}
    s.Graph.Progression.Gates={{beforeCell=cell,afterCell=s.Graph.Cells[key(4,3,0)]}}
    assert(not E:Placement(s.Graph,cell,id,'arena'),'progression transition rejects tether/cover source')
    s.Graph.Progression.Gates={}
    util.TraceHull=function() return {Hit=true,StartSolid=true} end
    assert(not E:Placement(s.Graph,cell,id,'arena'),'blocked native hull rejects tether/cover source')
    util.TraceHull=clearTrace
end
-- A passable screen needs local flank pockets, not a full maze loop.
...
assert(E:Placement(s.Graph,s.Graph.Cells[key(3,3,0)],'screenwright','arena'),'clear flank pockets admit screen without a graph cycle')
assert(E:Placement(s.Graph,s.Graph.Cells[key(3,3,0)],'towline','arena'),'tether retains ordinary corridor admission')
util.TraceHull=function(t)
    return {Hit=t.start:DistToSqr(t.endpos)>0,StartSolid=false,HitPos=t.endpos}
end
assert(not E:Placement(s.Graph,s.Graph.Cells[key(3,3,0)],'screenwright','arena'),'blocked lateral pocket rejects screen')
assert(E:Placement(s.Graph,s.Graph.Cells[key(3,3,0)],'towline','arena'),'screen clearance does not constrain tether')
util.TraceHull=clearTrace
s.Graph.Cells[key(3,3,0)].neighbors={}
assert(not E:Placement(s.Graph,s.Graph.Cells[key(3,3,0)],'screenwright','arena'),'screen requires a legal same-level exit')
s.Graph=openGraph
...
local fallback={id=904,cell=cell,cellKey=key(3,3,0),role='arena',sector=2,
    composition={towline=1,screenwright=1},entities={}}
assert(D:_SpawnEncounter(fallback) and #fallback.entities==2 and fallback.composition.shambler==2,'illegal tether/cover sources fall back without body proliferation')
...
local mixed={id=905,cell=cell,cellKey=key(3,3,0),role='arena',sector=2,
    composition={runner=1,wirewright=1,afterburst=1,carrion=1,towline=1,screenwright=1},entities={}}
assert(D:_SpawnEncounter(mixed) and #mixed.entities==6)
for i,id in ipairs({'runner','wirewright','afterburst','carrion','towline','screenwright'}) do
...
print('BESTIARY_B9_PRODUCTION_PASS: physical class-feat-HP identities; seeded replay; exact-life/graph/progression/campaign/freeze/death/disconnect; shared XP once; cap; production spawn/variance/progression; idempotence; complementary templates; safe/objective/transitions/blocked hull; screen lateral passage; bounded fallback')
