-- B6 production gate exercises progression, rewards, placement and canonical spawning.
...
local ids={'wirewright','snarer','cordon'}
local expected={wirewright={die=8,xp=50,magic=false},snarer={die=8,xp=50,magic=true},cordon={die=10,xp=55,magic=false}}
local magicClasses={}
for _,id in ipairs(ids) do
    ...
    for seed=1,16 do
        ...
        if def.magic then
            magicClasses[generated.classId]=true
            assert(C:_HasCapability({},generated,'offensive_magic_activation'),'Snarer receives usable casting feats')
            assert(C:_HasCapability({},generated,'magic_pool'),'Snarer has the canonical Magic-pool capability')
        else
...
assert(magicClasses.wizard,'Snarer can actually generate a Wizard build')
...
pair('wirewright');D.Entities={};D.activeCount=0
local creates=0
...
local encounter={id=903,cell=cell,cellKey=key(3,3,0),role='arena',sector=2,composition={wirewright=1,snarer=1,cordon=1},entities={}}
...
for _,name in ipairs({'wirewright_chase','snarer_detail','cordon_screen'}) do
    ...
    assert(not table.HasValue(D:_EligibleTemplates(3,'reward'),name),'trap specialist does not enter reward-only path')
    for seed=1,16 do
        local composition=D:_TemplateComposition(name,LOD.RNG.New(seed),4)
        for _,id in ipairs(ids) do
            if composition[id] then assert(composition[id]==1,'party/depth enrichment caps trap source') end
        end
    end
end
for _,id in ipairs(ids) do
    assert(E:Placement(s.Graph,cell,id,'arena'),'ordinary legal room admits trap source')
    s.Graph.CellTags[key(3,3,0)]={safe=true}
    assert(not E:Placement(s.Graph,cell,id,'arena'),'safe room rejects trap source')
    s.Graph.CellTags[key(3,3,0)]={objective=true}
    assert(not E:Placement(s.Graph,cell,id,'arena'),'objective room rejects trap source')
    s.Graph.CellTags[key(3,3,0)]={}
    s.Graph.VerticalEdges={{a=cell,b=s.Graph.Cells[key(4,3,0)]}}
    assert(not E:Placement(s.Graph,cell,id,'arena'),'vertical transition rejects trap source')
    s.Graph.VerticalEdges={}
    s.Graph.Progression.Gates={{beforeCell=cell,afterCell=s.Graph.Cells[key(4,3,0)]}}
    assert(not E:Placement(s.Graph,cell,id,'arena'),'progression transition rejects trap source')
    s.Graph.Progression.Gates={}
    util.TraceHull=function() return {Hit=true,StartSolid=true} end
    assert(not E:Placement(s.Graph,cell,id,'arena'),'blocked native hull rejects trap source')
    util.TraceHull=clearTrace
end
-- A stationary pressure node always preserves two legal exits and side pockets,
-- even if a diagnostic asks for reward placement outside its production templates.
local neighbors=cell.neighbors
cell.neighbors={[key(4,3,0)]=true}
assert(not E:Placement(s.Graph,cell,'cordon','arena') and not E:Placement(s.Graph,cell,'cordon','reward'),'Cordon rejects a one-exit trap room')
cell.neighbors=neighbors
local canTraverse=N.CanTraverse
N.CanTraverse=function(_,graph,from,to) return to==key(4,3,0) end
assert(not E:Placement(s.Graph,cell,'cordon','arena'),'geometric exits behind closed gates do not count')
N.CanTraverse=canTraverse
for _,sign in ipairs({-1,1}) do
    util.TraceHull=function(t)
        local delta=t.endpos-t.start
        return {Hit=delta:LengthSqr()>1 and delta.y*sign>0,HitPos=t.endpos}
    end
    assert(not E:Placement(s.Graph,cell,'cordon','arena'),'both side pockets must be traversable')
end
util.TraceHull=clearTrace
assert(E:Placement(s.Graph,cell,'cordon','arena'),'node placement restores after blockers clear')
...
local fallback={id=904,cell=cell,cellKey=key(3,3,0),role='arena',sector=2,
    composition={wirewright=1,snarer=1,cordon=1},entities={}}
assert(D:_SpawnEncounter(fallback) and #fallback.entities==3 and fallback.composition.shambler==3,'illegal trap sources fall back without body proliferation')
...
assert(D:_SpawnEncounter(fallback) and creates==6,'fallback retry cannot duplicate bodies')
print('BESTIARY_B6_PRODUCTION_PASS: own physical/Magic class-feat-HP identities; seeded replay; shared XP once; cap; real spawn/variance/progression; idempotence; complementary templates; safe/objective/transitions; two legal exits and clear pockets; bounded fallback')
