...
for id,d in pairs(E.Definitions) do
    ...
    if id~='climber' and id~='nodule' and not d.trap and not d.melee and not d.tactical and not d.mobile and not d.condition and not d.resource and not d.spacing and not d.crossfire and not d.discipline and not d.companion and not d.edict then E:Prepare(e);E:Begin(e,p,time);assert(e.LODRosterAttack and e.nw.LOD_RosterAttack==1);E:Interrupt(e);assert(not e.LODRosterAttack) end
end
