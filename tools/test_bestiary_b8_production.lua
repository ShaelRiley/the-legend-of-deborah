-- B8 production gate exercises progression, rewards, placement and canonical spawning.
...
local ids={'afterburst','carrion'}
local expected={afterburst={die=10,xp=50},carrion={die=8,xp=50}}
...
pair('afterburst');D.Entities={};D.activeCount=0
local creates=0
...
local encounter={id=903,cell=cell,cellKey=key(3,3,0),role='arena',sector=2,composition={afterburst=1,carrion=1},entities={}}
...
for _,name in ipairs({'afterburst_detail','carrion_feast'}) do
    assert(table.HasValue(D:_EligibleTemplates(2,'arena'),name) and table.HasValue(D:_EligibleTemplates(2,'ambush'),name))
    assert(table.HasValue(D:_EligibleTemplates(1,'arena'),name)==(name=='afterburst_detail'),'B27 permits only its explicit early directed identities')
...
local fallback={id=904,cell=cell,cellKey=key(3,3,0),role='arena',sector=2,
    composition={afterburst=1,carrion=1},entities={}}
assert(D:_SpawnEncounter(fallback) and #fallback.entities==2 and fallback.composition.shambler==2,'illegal melee sources fall back without body proliferation')
...
local mixed={id=905,cell=cell,cellKey=key(3,3,0),role='arena',sector=2,
    composition={runner=1,wirewright=1,afterburst=1,carrion=1},entities={}}
assert(D:_SpawnEncounter(mixed) and #mixed.entities==4)
for i,id in ipairs({'runner','wirewright','afterburst','carrion'}) do
...
print('BESTIARY_B8_PRODUCTION_PASS: physical class-feat-HP identities; seeded replay; shared XP once; cap; production spawn/variance/progression; idempotence; complementary templates; safe/objective/transitions/blocked hull; bounded fallback')
