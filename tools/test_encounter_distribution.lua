...
local b19={relay='soldier',lacemaker='runner'}
...
local b14={siphoner='runner',accumulator='soldier'}
local b13={outrider='soldier',conductor='shambler'}
local b12={absolver='shambler',exactor='flamer'}
 ...
 for variant=1,16 do
  ...
  for _,enc in ipairs(plan.encounters) do
   ...
   for id,count in pairs(enc.composition) do
    ...
    if b19[id] then
     assert(enc.sector>=2 and (enc.role=='arena' or enc.role=='ambush'),'B19 production path')
     assert(count==1 and (enc.composition[b19[id]] or 0)>=1,'B19 singleton and complementary companion')
    end
    if b18[id] then
    ...
    if b16[id] then
     assert(enc.sector>=2 and (enc.role=='arena' or enc.role=='ambush'),'B16 production path')
     assert(count==1 and (enc.composition[b16[id]] or 0)>=1,'B16 singleton and complementary companion')
    end
    if b15[id] then
     assert(enc.sector>=2 and (enc.role=='arena' or enc.role=='ambush'),'B15 production path')
     assert(count==1 and (enc.composition[b15[id]] or 0)>=1,'B15 singleton and complementary companion')
    end
    if b14[id] then
     assert(enc.sector>=2 and (enc.role=='arena' or enc.role=='ambush'),'B14 production path')
     assert(count==1 and (enc.composition[b14[id]] or 0)>=1,'B14 singleton and complementary companion')
    end
    if b13[id] then
     assert(enc.sector>=2 and (enc.role=='arena' or enc.role=='ambush'),'B13 production path')
     assert(count==1 and (enc.composition[b13[id]] or 0)>=1,'B13 singleton and complementary companion')
    end
    if b12[id] then
     assert(enc.sector>=2 and (enc.role=='arena' or enc.role=='ambush'),'B12 production path')
     assert(count==1 and (enc.composition[b12[id]] or 0)>=(id=='absolver' and 2 or 1),'B12 singleton and complementary companion')
    end
    if b11[id] then
     assert(enc.sector>=2 and (enc.role=='arena' or enc.role=='ambush'),'B11 production path')
     assert(count==1 and (enc.composition[b11[id]] or 0)>=1,'B11 singleton and complementary companion')
    end
    if b10[id] then
