-- B19 production gate exercises progression, rewards, placement and canonical spawning.
...
assert(#order==61 and order[56]=='interposer' and order[57]=='mourner'
    and order[58]=='censor' and order[59]=='surveyor'
    and order[60]=='relay' and order[61]=='lacemaker','append-only production spawn ordinals60/61')
local expected={
    relay={die=8,xp=50,magic=false,abilities={11,13,11,13,12,10},fighter=35,rogue=65,wizard=0,morale=5},
    lacemaker={die=8,xp=50,magic=false,abilities={12,12,12,11,12,10},fighter=60,rogue=40,wizard=0,morale=5}
}
...
pair('relay');D.Entities={};D.activeCount=0
local creates=0
...
local encounter={id=903,cell=cell,cellKey=key(3,3,0),role='arena',sector=2,composition={relay=1,lacemaker=1},entities={}}
...
for _,name in ipairs({'relay_detail','lacemaker_detail'}) do
    ...
    assert(not table.HasValue(D:_EligibleTemplates(3,'reward'),name),'companion-link specialist does not enter reward-only path')
    for seed=1,16 do
        local composition=D:_TemplateComposition(name,LOD.RNG.New(seed),4)
        local companion=name=='relay_detail' and 'soldier' or 'runner'
        assert((composition[companion] or 0)>=1,'authored complementary ordinary body survives enrichment')
        for _,id in ipairs(ids) do
            if composition[id] then assert(composition[id]==1,'party/depth enrichment caps companion-link source') end
        end
    end
end
for _,id in ipairs(ids) do
    assert(E:Placement(s.Graph,cell,id,'arena'),'ordinary legal room admits companion-link source')
    s.Graph.CellTags[key(3,3,0)]={safe=true}
    assert(not E:Placement(s.Graph,cell,id,'arena'),'safe room rejects companion-link source')
    s.Graph.CellTags[key(3,3,0)]={objective=true}
    assert(not E:Placement(s.Graph,cell,id,'arena'),'objective room rejects companion-link source')
    s.Graph.CellTags[key(3,3,0)]={}
    s.Graph.VerticalEdges={{a=cell,b=s.Graph.Cells[key(4,3,0)]}}
    assert(not E:Placement(s.Graph,cell,id,'arena'),'vertical transition rejects companion-link source')
    s.Graph.VerticalEdges={}
    s.Graph.Progression.Gates={{beforeCell=cell,afterCell=s.Graph.Cells[key(4,3,0)]}}
    assert(not E:Placement(s.Graph,cell,id,'arena'),'progression transition rejects companion-link source')
    s.Graph.Progression.Gates={}
    util.TraceHull=function() return {Hit=true,StartSolid=true} end
    assert(not E:Placement(s.Graph,cell,id,'arena'),'blocked native hull rejects companion-link source')
    util.TraceHull=clearTrace
end
-- Both companion-link identities require lateral room and a legal same-floor exit.
for _,id in ipairs(ids) do
    local c=s.Graph.Cells[key(3,3,0)]
    local neighbors=c.neighbors;c.neighbors={}
    assert(not E:Placement(s.Graph,c,id,'arena'),'no isolated companion-link spawn')
...
-- Link placement uses exactly two96-unit lateral routes; blocked approaches
-- cannot be rescued by a companion or by ordinary encounter enrichment.
for _,id in ipairs(ids) do
    util.TraceHull=function(t) return {Hit=t.start:Distance(t.endpos)>96.01,StartSolid=false,HitPos=t.endpos} end
    assert(E:Placement(s.Graph,cell,id,'arena'),'authored96-unit lateral route suffices')
    util.TraceHull=function(t) return {Hit=t.start:Distance(t.endpos)>95.99,StartSolid=false,HitPos=t.endpos} end
    assert(not E:Placement(s.Graph,cell,id,'arena'),'shortened lateral route rejects link source')
end
util.TraceHull=clearTrace
...
local fallback={id=904,cell=cell,cellKey=key(3,3,0),role='arena',sector=2,
    composition={relay=1,lacemaker=1},entities={}}
assert(D:_SpawnEncounter(fallback) and #fallback.entities==2 and fallback.composition.shambler==2,'illegal companion-link sources fall back without body proliferation')
...
local mixed={id=905,cell=cell,cellKey=key(3,3,0),role='arena',sector=2,
    composition={runner=1,wirewright=1,afterburst=1,carrion=1,towline=1,screenwright=1,listener=1,shy=1,absolver=1,exactor=1,outrider=1,conductor=1,siphoner=1,accumulator=1,fusilier=1,bombardier=1,halter=1,pacer=1,interposer=1,mourner=1,censor=1,surveyor=1,relay=1,lacemaker=1},entities={}}
assert(D:_SpawnEncounter(mixed) and #mixed.entities==24)
for i,id in ipairs({'runner','wirewright','afterburst','carrion','towline','screenwright','listener','shy','absolver','exactor','outrider','conductor','siphoner','accumulator','fusilier','bombardier','halter','pacer','interposer','mourner','censor','surveyor','relay','lacemaker'}) do
...
print('BESTIARY_B19_PRODUCTION_PASS: physical class-feat-HP identities; seeded replay; exact-life/graph/progression/campaign/freeze/death/disconnect; shared XP once; cap; production spawn/variance/progression; idempotence; complementary templates; safe/objective/transitions/blocked hull; companion-link admission and96-unit escape pockets; bounded fallback')
