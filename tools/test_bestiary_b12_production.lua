-- B12 production gate exercises progression, rewards, placement and canonical spawning.
...
local ids={'absolver','exactor'}
local expected={absolver={die=8,xp=50,magic=true},exactor={die=10,xp=55,magic=false}}
...
pair('absolver');D.Entities={};D.activeCount=0
local creates=0
...
local encounter={id=903,cell=cell,cellKey=key(3,3,0),role='arena',sector=2,composition={absolver=1,exactor=1},entities={}}
...
for _,name in ipairs({'absolver_detail','exactor_pressure'}) do
    ...
    assert(not table.HasValue(D:_EligibleTemplates(3,'reward'),name),'condition-interaction specialist does not enter reward-only path')
    for seed=1,16 do
        local composition=D:_TemplateComposition(name,LOD.RNG.New(seed),4)
        for _,id in ipairs(ids) do
            if composition[id] then assert(composition[id]==1,'party/depth enrichment caps condition-interaction source') end
        end
    end
end
for _,id in ipairs(ids) do
    assert(E:Placement(s.Graph,cell,id,'arena'),'ordinary legal room admits condition-interaction source')
    s.Graph.CellTags[key(3,3,0)]={safe=true}
    assert(not E:Placement(s.Graph,cell,id,'arena'),'safe room rejects condition-interaction source')
    s.Graph.CellTags[key(3,3,0)]={objective=true}
    assert(not E:Placement(s.Graph,cell,id,'arena'),'objective room rejects condition-interaction source')
    s.Graph.CellTags[key(3,3,0)]={}
    s.Graph.VerticalEdges={{a=cell,b=s.Graph.Cells[key(4,3,0)]}}
    assert(not E:Placement(s.Graph,cell,id,'arena'),'vertical transition rejects condition-interaction source')
    s.Graph.VerticalEdges={}
    s.Graph.Progression.Gates={{beforeCell=cell,afterCell=s.Graph.Cells[key(4,3,0)]}}
    assert(not E:Placement(s.Graph,cell,id,'arena'),'progression transition rejects condition-interaction source')
    s.Graph.Progression.Gates={}
    util.TraceHull=function() return {Hit=true,StartSolid=true} end
    assert(not E:Placement(s.Graph,cell,id,'arena'),'blocked native hull rejects condition-interaction source')
    util.TraceHull=clearTrace
end
-- Exactor requires lateral room and a legal same-floor exit.
for _,id in ipairs({'exactor'}) do
    local c=s.Graph.Cells[key(3,3,0)]
    local neighbors=c.neighbors;c.neighbors={}
    assert(not E:Placement(s.Graph,c,id,'arena'),'no isolated condition-interaction spawn')
...
local fallback={id=904,cell=cell,cellKey=key(3,3,0),role='arena',sector=2,
    composition={absolver=1,exactor=1},entities={}}
assert(D:_SpawnEncounter(fallback) and #fallback.entities==2 and fallback.composition.shambler==2,'illegal condition-interaction sources fall back without body proliferation')
...
local mixed={id=905,cell=cell,cellKey=key(3,3,0),role='arena',sector=2,
    composition={runner=1,wirewright=1,afterburst=1,carrion=1,towline=1,screenwright=1,listener=1,shy=1,absolver=1,exactor=1},entities={}}
assert(D:_SpawnEncounter(mixed) and #mixed.entities==10)
for i,id in ipairs({'runner','wirewright','afterburst','carrion','towline','screenwright','listener','shy','absolver','exactor'}) do
...
print('BESTIARY_B12_PRODUCTION_PASS: physical class-feat-HP identities; seeded replay; exact-life/graph/progression/campaign/freeze/death/disconnect; shared XP once; cap; production spawn/variance/progression; idempotence; complementary templates; safe/objective/transitions/blocked hull; condition support and escape pockets; bounded fallback')
