-- B5 production gate exercises progression, rewards, placement and canonical spawning.
...
local ids={'caromer','reeler','forker'}
local expected={caromer={die=8,xp=50},reeler={die=8,xp=50},forker={die=10,xp=55}}
...
pair('caromer');D.Entities={};D.activeCount=0
local creates=0
...
local encounter={id=903,cell=cell,cellKey=key(3,3,0),role='arena',sector=2,composition={caromer=1,reeler=1,forker=1},entities={}}
...
for _,name in ipairs({'caromer_screen','reeler_chase','forker_crossfire'}) do
    ...
    for seed=1,16 do
        ...
        for _,id in ipairs(ids) do
            if composition[id] then assert(composition[id]==1,'party/depth enrichment caps projectile-pattern source') end
        end
    end
end
for _,id in ipairs(ids) do
    assert(E:Placement(s.Graph,cell,id,'arena'),'ordinary legal room admits projectile-pattern source')
    s.Graph.CellTags[key(3,3,0)]={safe=true}
    assert(not E:Placement(s.Graph,cell,id,'arena'),'safe room rejects projectile-pattern source')
    s.Graph.CellTags[key(3,3,0)]={}
    s.Graph.Progression.Gates={{beforeCell=cell,afterCell=s.Graph.Cells[key(4,3,0)]}}
    assert(not E:Placement(s.Graph,cell,id,'arena'),'progression transition rejects projectile-pattern source')
    s.Graph.Progression.Gates={}
    util.TraceHull=function() return {Hit=true,StartSolid=true} end
    assert(not E:Placement(s.Graph,cell,id,'arena'),'blocked native hull rejects projectile-pattern source')
    util.TraceHull=clearTrace
end
print('BESTIARY_B5_PRODUCTION_PASS: own nonmagical class/feat/HP identities; seeded replay; capability eligibility; shared XP once; cap; real spawn/variance/progression; idempotence; complementary templates')
