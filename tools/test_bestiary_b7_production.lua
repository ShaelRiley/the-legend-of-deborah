-- B7 production gate exercises progression, rewards, placement and canonical spawning.
...
local ids={'reaper','drubber','fencer'}
local expected={reaper={die=10,xp=55},drubber={die=10,xp=55},fencer={die=8,xp=50}}
...
pair('reaper');D.Entities={};D.activeCount=0
local creates=0
...
local encounter={id=903,cell=cell,cellKey=key(3,3,0),role='arena',sector=2,composition={reaper=1,drubber=1,fencer=1},entities={}}
...
for _,name in ipairs({'reaper_detail','drubber_chase','fencer_screen'}) do
    assert(table.HasValue(D:_EligibleTemplates(2,'arena'),name) and table.HasValue(D:_EligibleTemplates(2,'ambush'),name))
    assert(table.HasValue(D:_EligibleTemplates(1,'arena'),name)==(name=='reaper_detail' or name=='drubber_chase'),'B27 permits only its explicit early directed identities')
    assert(not table.HasValue(D:_EligibleTemplates(3,'reward'),name),'melee specialist does not enter reward-only path')
    for seed=1,16 do
        local composition=D:_TemplateComposition(name,LOD.RNG.New(seed),4)
        for _,id in ipairs(ids) do
            if composition[id] then assert(composition[id]==1,'party/depth enrichment caps melee source') end
        end
    end
end
for _,id in ipairs(ids) do
    assert(E:Placement(s.Graph,cell,id,'arena'),'ordinary legal room admits melee source')
    s.Graph.CellTags[key(3,3,0)]={safe=true}
    assert(not E:Placement(s.Graph,cell,id,'arena'),'safe room rejects melee source')
    s.Graph.CellTags[key(3,3,0)]={objective=true}
    assert(not E:Placement(s.Graph,cell,id,'arena'),'objective room rejects melee source')
    s.Graph.CellTags[key(3,3,0)]={}
    s.Graph.VerticalEdges={{a=cell,b=s.Graph.Cells[key(4,3,0)]}}
    assert(not E:Placement(s.Graph,cell,id,'arena'),'vertical transition rejects melee source')
    s.Graph.VerticalEdges={}
    s.Graph.Progression.Gates={{beforeCell=cell,afterCell=s.Graph.Cells[key(4,3,0)]}}
    assert(not E:Placement(s.Graph,cell,id,'arena'),'progression transition rejects melee source')
    s.Graph.Progression.Gates={}
    util.TraceHull=function() return {Hit=true,StartSolid=true} end
    assert(not E:Placement(s.Graph,cell,id,'arena'),'blocked native hull rejects melee source')
...
local fallback={id=904,cell=cell,cellKey=key(3,3,0),role='arena',sector=2,
    composition={reaper=1,drubber=1,fencer=1},entities={}}
assert(D:_SpawnEncounter(fallback) and #fallback.entities==3 and fallback.composition.shambler==3,'illegal melee sources fall back without body proliferation')
...
local mixed={id=905,cell=cell,cellKey=key(3,3,0),role='arena',sector=2,
    composition={runner=1,wirewright=1,reaper=1,drubber=1,fencer=1},entities={}}
assert(D:_SpawnEncounter(mixed) and #mixed.entities==5)
for i,id in ipairs({'runner','wirewright','reaper','drubber','fencer'}) do
...
print('BESTIARY_B7_PRODUCTION_PASS: physical class-feat-HP identities; seeded replay; shared XP once; cap; production spawn/variance/progression; idempotence; complementary templates; safe/objective/transitions/blocked hull; bounded fallback')
