...
function ENT:Initialize()
    ...
    self:SetModel(archetype=='vending_machine' and 'models/props_interiors/VendingMachineSoda01a.mdl'
        or chest and 'models/Items/item_item_crate.mdl' or 'models/props_lab/reciever01b.mdl')
    self:SetMoveType(MOVETYPE_NONE)
    self:SetSolid(SOLID_BBOX)
    if archetype=='vending_machine' then self:SetCollisionBounds(Vector(-24,-20,0),Vector(24,20,80))
...
function ENT:Use(ply)
    ...
    if not ok and IsValid(ply) and ply:IsPlayer() and CurTime()>=(self.LODNextNotice or 0) then
        ...
        ply:ChatPrint((vending and 'DEBBIE VENDING — ' or archetype=='treasure_chest' and 'TREASURE CHEST — ' or chest and 'LOCKED CHEST — ' or 'DEBBIE SLOTS — ')..message)
    end
end
