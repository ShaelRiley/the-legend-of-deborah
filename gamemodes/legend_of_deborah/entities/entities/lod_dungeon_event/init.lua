...
function ENT:Initialize()
    ...
    self:SetModel(archetype=='equipment_quiz' and 'models/monk.mdl'
    ...
    if archetype=='equipment_quiz' then self:SetCollisionBounds(Vector(-18,-18,0),Vector(18,18,74))
    ...
    self:SetColor(archetype=='equipment_quiz' and Color(196,60,60) or archetype=='warp_hole' and Color(70,135,155) or Color(205,175,75))
end
function ENT:Use(ply)
    ...
    if not ok and IsValid(ply) and ply:IsPlayer() and CurTime()>=(self.LODNextNotice or 0) then
        ...
        local message=reason=='already' and (archetype=='equipment_quiz' and 'Your Game Master attempt is spent for this dungeon.' or vending and 'You already bought your potion in this dungeon.' or chest and 'You already claimed this chest in this dungeon.' or 'You already played this machine in this dungeon.')
            or reason=='storage' and 'Wallet unavailable. Nothing spent; try again.'
            or type(reason)=='string' and reason or 'Machine unavailable.'
        ply:ChatPrint((archetype=='equipment_quiz' and 'GAME MASTER — ' or archetype=='bribe_blockade' and 'BRIBE BLOCKADE — ' or archetype=='warp_hole' and 'WARP HOLE — ' or vending and 'DEBBIE VENDING — ' or archetype=='treasure_chest' and 'TREASURE CHEST — ' or chest and 'LOCKED CHEST — ' or 'DEBBIE SLOTS — ')..message)
    end
end
