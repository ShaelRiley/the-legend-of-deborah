...
local DAMAGE_PROFILE = {label = "CROWBAR", source = "crowbar", count = 1, sides = 8}
...
function SWEP:PrimaryAttack()
    ...
    self:EmitSound("weapons/crowbar/crowbar_swing1.wav", 72, 100, 0.9, CHAN_WEAPON)
    ...
    if not IsValid(target) or not target.LODHostile or target.LODDead then return end

    local rolls = LOD and LOD.CombatRolls
    if not rolls or not rolls._RNG or not rolls._RollFormula then return end
    ...
    if rolls._Send and rolls._DamageEventText then
        ...
        rolls:_Send(owner, 0, rolls:_DamageEventText(owner, "1d8", total,
    ...
    self:EmitSound("weapons/crowbar/crowbar_impact1.wav", 74, 100, 0.9, CHAN_WEAPON)
end
