...
function SWEP:PrimaryAttack()
    ...
    local aim = LOD and LOD.UniversalAim
    if aim and aim.CommitPrimaryAttack then
        aimMultiplier = select(1, aim:CommitPrimaryAttack(owner, "weapon_lod_crowbar"))
    elseif LOD and LOD.ConsumeAimState then
        aimMultiplier = LOD.ConsumeAimState(owner, self)
    end
    ...
    if total < healthBefore and IsValid(target) and not target.LODDead
        ...
        LOD.M3HitFeedback:ApplyHitStun(target, 1, owner, nil, "melee", tags.attackEvent)
    end
