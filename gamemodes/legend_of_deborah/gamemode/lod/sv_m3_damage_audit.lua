...
concommand.Add("lod_m3_damage_audit", function(ply)
    ...
    for _, id in ipairs({"shambler", "runner", "soldier"}) do
        ...
        if cfg then
            ...
            tell(ply, string.format("%s baseHP=%d engine-cvar body-shot estimate=%s", id, cfg.baseHP or -1, estimate))
        end
    end
    ...
    for _, ent in ipairs(ents.FindByClass("lod_hostile")) do
        if IsValid(ent) then
            found = found + 1
            tell(ply, string.format("live %s hp=%d/%d lastPlayerDamage=%s weapon=%s",
                tostring(ent.LODArchetypeId), ent:Health(), ent:GetMaxHealth(),
