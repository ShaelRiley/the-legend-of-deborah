# Implementation Map — 2026-08-26 Gate D
...
| Watcher support archetype | `sv_watcher.lua`, `sv_watcher_attack_guard.lua`, `sv_watcher_committed_retreat.lua`, `cl_watcher.lua`; Scanner-derived no-direct-attack support unit, 1.25 s LOS scan with escalating beam/audio, completed scan alerts only already-existing same-floor wanderers within 6 graph cells, bullet hit-stun/LOS break cancels, no reinforcement spawning; committed post-scan/too-close range recovery; rare wandering weight 4 plus Sector-2+ `Surveillance = 1 Watcher + 2 Shamblers`; scan/alert contract runtime-accepted |
| Seeker momentum archetype | `sv_seeker.lua`, `sv_seeker_personality.lua`, `sv_seeker_sound_safety.lua`, `sv_seeker_spawn_reliability.lua`, `sv_device_motion_safety.lua`, `cl_seeker.lua`, `entities/entities/lod_hostile/cl_init.lua`; Rollermine-derived wind-up/straight charge, wall-impact stun, 150-unit minimum attack range, committed medium-range retreat; seeded 30% left/right pre-charge feint and 25% eligible post-hit same-cell orbit; flourish timer exists only while active and all displacement still uses Motion V2; visible Rollermine rotation is client-side and derived from actual horizontal travel distance so spin rate rises with movement speed; core charge/retreat/floor-safety runtime accepted, personality polish awaiting runtime acceptance |
...
| Crowbar | `entities/weapons/weapon_lod_crowbar/`, `entities/entities/lod_hero_crowbar_pulse/`, `sv_rpg_gate_e_crowbar.lua`: baseline `1d3`, Bash exploding `1d6` + 168 push, Walloper SUPER `1d12`, Wrecking extra wall-crush die, and one globally exclusive glowing Hero crowbar at Bio Blaster speed; 96-unit base reach and existing hit feedback retained |
...
| Expanded normal roster | **Gate D current: Watcher scan/alert accepted; Seeker core accepted, movement-personality polish awaiting acceptance.** Next after Seeker: `Sentry → Razor → Flamer → Big Crab → Arc Caster → Lurker → Beam Sweeper` |
...
- Crowbar: baseline `1d3`, Bash exploding `1d6`, Walloper SUPER `1d12`; 96-unit reach before Long Reach, Bash/Walloper 168 push, Wrecking extra wall-crush die, Hero projectile range `max(1, WIS bonus)` 384-unit cells.
- Pistol: `1d4`.
...
- .357 Magnum: each projectile deals `1d12!+X`, with X equal to the number of chambers already empty before that trigger. A normal six-round cylinder therefore progresses **+0,+1,+2,+3,+4,+5** and reload resets the sequence. Trigger 5 fires a rapid **two-projectile** Magnum burst; trigger 6 fires a rapid **three-projectile** burst. Below 60 current HP, each trigger has exactly one `(60-HP)%` check to add one further free Magnum projectile, so any normal/burst shot can grow by one projectile. On a final-cartridge trigger below 34% of max Health, the final cartridge has `floor(34-currentHealthPercent)%` chance not to be consumed; preservation is applied after the full burst so every projectile retains chamber-6/+5 state, and the next trigger is another final-chamber shot. Aim State arms after 0.5 seconds of complete player/aim stillness and is canceled by movement or aim change before firing; an aimed trigger resolves at **×2 damage across the complete trigger**, including all free burst projectiles, cylinder bonus, each projectile's Boomchain, and every fresh Boomchain added by deeper penetration. Extra projectiles consume no additional cartridge, use the triggering chamber's X bonus, roll independent d12 Boomchains, and retain normal piercing. Each deeper pierced target adds one fresh independent d12 Boomchain; geometry remains authoritative and total targets remain capped at eight.
...
- Seeker `3d4+3` **provisional implementation tuning; GDD does not prescribe a numeric Seeker health formula**
...
- Seeker: no ordinary melee; charge uses its dedicated dice-era impact contract
...
- Seeker flourish logic that bypasses Motion V2, crosses graph-blocked architecture, or creates a permanent general-purpose AI polling loop.
