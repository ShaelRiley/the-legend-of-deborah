...
Neil + The Brute and Gordon remain deliberately deferred. The current nine-stage dungeon progression is acceptable as a temporary multiplayer test harness even though the live GDD now inserts Neil + The Brute after the Blue Gate.
...
`sv_multiplayer_hardening.lua` adds cross-system lifecycle guarantees needed before the first real multiplayer run:
...
5. Play far enough to confirm ordinary movement, combat, loot, map/Magic, and progression still work.
6. Die once, play some Tetris, and verify the respawn does **not** become available before 20 seconds even after clearing lines.
...
Run `lod_multiplayer_status` after both clients are active.
...
No targeting deadlocks, teleporting, cross-floor wall shooting, permanent stuck states, or material performance collapse.
...
4. implement Neil + The Brute;
5. implement Gordon the Warden;
